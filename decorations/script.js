...
function createCard(item) {
    ...
    const area = parseInt(item.width) * parseInt(item.height);
    const po = getPO(item);
    const poPerTile = area > 0 ? (po / area).toFixed(2) : "N/A";
    const might = item.mightValue || "0";
    const fusion = (item.isFusionSource === "1" || item.isFusionTarget === "1") ? 
                   (item.isFusionSource === "1" && item.isFusionTarget === "1" ? "target & source" : 
                    item.isFusionSource === "1" ? "source" : "target") : "-";
    const sellPrice = item.sellC1 || "0";
    const source = [item.comment1, item.comment2].filter(Boolean).join(", ");
    const id = item.wodID || "???";

    return `
  <div class="col-lg-4 col-md-6 col-12 deco-card" data-name="${name.toLowerCase()}" data-size="${size}" data-po="${po}" data-pot="${poPerTile}">
    <div class="card mb-3">
      <div class="card-body">
        ...
        <div class="row">
          <div class="col-12">
            ...
            <p><strong>Comments:</strong> ${source || "-"}</p>
          </div>
        </div>
      </div>
    </div>
