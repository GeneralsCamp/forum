  ...
  fetch('data.json')
    ...
    .then(data => {
      ...
      units.forEach(unit => {
        ...
        if (unit.id === "unit8") {
          unit.meleeCombatStrength = 370 + (attackBasics.beefMeleeLevel * 5);
        }
        if (unit.id === "unit7") {
          unit.rangedCombatStrength = 390 + (attackBasics.beefRangeLevel * 5);
        }
        if (unit.id === "unit10") {
          unit.meleeCombatStrength = 430 + (attackBasics.beefVeteranMeleeLevel * 5);
        }
        if (unit.id === "unit9") {
          unit.rangedCombatStrength = 450 + (attackBasics.beefVeteranRangeLevel * 5);
        }
        unitStats.push({
...
window.addEventListener('load', () => {
  ...
  attackBasics.beefRangeLevel = attackBasics.beefRangeLevel || 10;
  attackBasics.beefMeleeLevel = attackBasics.beefMeleeLevel || 10;

  attackBasics.beefVeteranRangeLevel = attackBasics.meadRangeLevel || 10;
  attackBasics.beefVeteranMeleeLevel = attackBasics.meadMeleeLevel || 10;
});
...
function initializeUnits(units) {
  ...
  units.forEach((unit, index) => {
    ...
    if (unit.id === "unit1") {
      ...
      levelInfo = `(Lv.${attackBasics.beefRangeLevel})`;
    } else if (unit.id === "unit8") {
      levelInfo = `(Lv.${attackBasics.beefMeleeLevel})`;
    } else if (unit.id === "unit9") {
      levelInfo = `(Lv.${attackBasics.beefVeteranRangeLevel})`;
    } else if (unit.id === "unit10") {
      levelInfo = `(Lv.${attackBasics.beefVeteranMeleeLevel})`;
    }
  ...
  tools.forEach((tool, index) => {
    ...
    if (toolRange && toolValue) {
      ...
      if (tool.toolLimit && tool.toolLimit > 0) {
        ...
        ['left', 'front', 'right'].forEach(s => {
          if (totalTools[s] && totalTools[s][waveIndex - 1]) {
            totalUsedThisWave += (totalTools[s][waveIndex - 1].find(t => t.type === `Tool${tool.id.charAt(tool.id.length - 1)}`)?.count || 0);
          }
        });
...
function openBasicsModal() {
  ...
  document.getElementById('confirmBasics').onclick = function () {
    ...
    attackBasics.beefRangeLevel = parseInt(document.getElementById('beef-range-level-slider').value);
    attackBasics.beefMeleeLevel = parseInt(document.getElementById('beef-melee-level-slider').value);
    attackBasics.beefVeteranRangeLevel = parseInt(document.getElementById('beef-veteran-range-level-slider').value);
    attackBasics.beefVeteranMeleeLevel = parseInt(document.getElementById('beef-veteran-melee-level-slider').value);
